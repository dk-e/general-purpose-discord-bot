...
const commandBuilder = new SlashCommandBuilder().setName("mp3").setDescription("Download a YouTube video as an MP3 file.");
...
const command = async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
        const response = await fetch("https://media-download-api.onrender.com/youtube/downloadMp3", {
            method: "POST",
            headers: {
        ...
        await interaction.reply({ content: `There was an error fetching the MP3 file: ${error.message}`, ephemeral: true });
    }
};
